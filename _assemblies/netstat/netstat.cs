...
namespace MyNetstat
{
	class Program    
	{
		static void Main(string[] args)
		{
			...
			// Print TCP Connections and State
			TcpConnectionInformation[] tcpConnections = ipProperties.GetActiveTcpConnections();
			foreach (TcpConnectionInformation info in tcpConnections)
			{
				string tproto    = "TCP";
				string laddress  = info.LocalEndPoint.Address.ToString() + ":" + info.LocalEndPoint.Port.ToString();
				string raddress  = info.RemoteEndPoint.Address.ToString() + ":" + info.RemoteEndPoint.Port.ToString();
				string cstate    = info.State.ToString();
				string data = String.Format("{0,-7}{1,-23}{2,-23}{3,-16}", tproto, laddress, raddress, cstate);
